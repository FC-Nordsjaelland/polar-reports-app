...
if authorization_code and submitted:
    ...
    try:
        ...
        st.sidebar.markdown(
            "### Session hasn't been named yet. Please name the session in the Polar system."
        )
        st.stop()
    ...
    if len(list(inv_sessions_dict)) == 0:
        ...
        for i in list(inv_sessions_dict.keys()):
            time_map_session.append(
                [
                    i,
                    i.split("(")[-1].split(",")[-1].split("-")[0].strip()[0:2],
                    i.split("(")[-1].split(",")[-1].split("-")[0].strip()[3:],
                ]
            )

        df_sessions_day = pd.DataFrame(
            time_map_session, columns=["List of training sessions", "hour", "minute"]
        )
        ...
        selected_session_name = training_sessions_lst[
            no_of_session
        ]  # first session in a day = 1, second = 2, etc
        ...
        df_volume_table = df_volume_table[
            ~df_volume_table.iloc[:, 0].str.contains("Åvg.|Avg.")
        ]

        # clean decimals
        df_volume_table.iloc[:, :-1] = df_volume_table.iloc[:, :-1].astype(
            int, errors="ignore"
        )
        ...
        previous_activity_start_time = convert_datetime_to_string(
        ...
        df_plot_clean_previous = (
            df_plot_previous.groupby(["position_name", "athlete_name"])
            .sum()
            .div(len(sessions_interval_dict))
        )

        df_plot_clean_previous = df_plot_clean_previous.join(
            df_max_speed, on=["position_name", "athlete_name"]
        )
        ...
        # %%
        # prep dataframe for table
        df_volume_table_previous = df_plot_clean_previous.reset_index().drop(
            "position_name", axis=1
        )
        df_volume_table_previous = df_volume_table_previous[
            ~df_volume_table_previous.iloc[:, 0].str.contains("Åvg.|Avg.")
        ]

        # clean decimals
        df_volume_table_previous.iloc[:, :-1] = df_volume_table_previous.iloc[
            :, :-1
        ...
        with PdfPages(
            f"Physical Report {selected_session_name.split('(')[0]}.pdf"
        ) as pdf:
            # plot
            plot_day = plot_physical_volume(
                df_plot_clean,
            ...
            ## TABLES
            # plot table
            daily_table = plot_table(
                df_volume_table,
                main_col="Total Distance",
            ...
            # plot table
            weekly_table = plot_table(
                df_volume_table_previous,
                main_col="Total Distance",
                title=f"Rolling 7 days\n{previous_activity_start_time}  to  {activity_start_time}",
            )
            col2.pyplot(weekly_table)
            pdf.savefig(bbox_inches="tight")

        with open(
            f"Physical Report {selected_session_name.split('(')[0]}.pdf", "rb"
        ) as report:
            st.download_button(
                "Download Report",
                report,
        ...
        col2.dataframe(
            df_plot_clean_previous.style.background_gradient().format(precision=1)
        )
        ...
        if account == "M":
            ...
            altair_volume_plot = plot_altair_scatter(
                df_altair_plot,
                title="Daily Volume Load",
                x="Total Distance:Q",
            ...
            # Internal
            altair_internal_plot = plot_altair_scatter(
                df_altair_plot,
                title="Daily Internal Load",
                x="Sprint distance (>25km/h):Q",
                y="HR (>85%):Q",
                tooltip=[
                    "athlete_name:N",
