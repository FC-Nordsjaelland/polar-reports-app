    ...
    if account == "M":
        ...
        data = pd.concat(
            [
                data,
                pd.Series(
                    data["Duration"].astype(str).apply(hms_to_m), name="Duration"
                ),
            ],
            axis=1,
        )
...
st.markdown(
    "1) Right click, copy and open the [link](%s)" % link, unsafe_allow_html=True
)
st.markdown(
...
with st.form(key="my_form"):
    ...
    activity_date = st.date_input(
        "Choose a training session's date", datetime.date.today()
    )
...
if authorization_code and submitted:
    ...
    year, month, day = str(activity_date).split("-")
    ...
    girls_template = pd.read_csv(
        parent + "/utils/templates/CSV_girls_template.csv", delimiter=";"
    )
