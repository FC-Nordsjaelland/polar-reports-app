...
from kitbag.plots.bar_charts import plot_bars, plot_physical_volume
...
def session_data(selected_session_id, selected_session_name, activity_data=False):
    ...
    session_data = preprocess(
        session_data, date_time, players, selected_session_name, account
    )
...
with st.sidebar.form("my_form"):
    ...
    date_range = st.date_input(
        label="Select date range", value=[seven_days_ago, today_date]
    )
...
if chosen_team in ["Superliga", "RTD senior", "U19", "U17", "U15"]:
    ...
    if chosen_team == "Superliga":
        ...
        chosen_team_long = "RTD Ghana - Senior"
...
if authorization_code:
    ...
    sessions_data = api.get_sessions(
        tokens=tokens, team_id=team_id, dates=[date1, date2]
    )
    ...
    for i, j in sessions_interval_dict.items():
        if type(j) != str:
            st.sidebar.markdown(
                f"### At least one session in that range is not named. Please name the session in Polar."
            )
            st.stop()
    ...
    df_stats_gameweek_daily = df_stats_clean_previous.groupby(
        ["position_name", "athlete_name"]
    ).sum()
    df_stats_gameweek_daily = df_stats_gameweek_daily.drop(
        "Maximum speed [km/h]", axis=1
    )
    df_stats_gameweek_daily["Total Distance (km)"] = (
        df_stats_gameweek_daily["Total Distance"] / 1000
    )
    df_stats_gameweek_daily["Total Distance (km)"] = df_stats_gameweek_daily[
        "Total Distance (km)"
    ].round(1)
    ...
    df_plot_clean_previous = df_plot_clean_previous.reindex(
        position_order, axis=0, level=0
    )

    # prep dataframe for table
    df_team_volume_table = df_plot_clean_previous.reset_index().drop(
        "position_name", axis=1
    )
    df_team_volume_table = df_team_volume_table[
        ~df_team_volume_table.iloc[:, 0].str.contains("Åvg.|Avg.")
    ]

    # clean decimals
    df_team_volume_table.iloc[:, :-1] = df_team_volume_table.iloc[:, :-1].astype(
        int, errors="ignore"
    )
    ...
    name_time_session_map_df = name_time_session_map_df.drop(
        ["position_name", "athlete_name"], axis=1
    )
    name_time_session_map_df = name_time_session_map_df.drop_duplicates(
        subset=["Session name"]
    )
    ...
    df_stats_gameweek_daily = pd.merge(
        df_stats_gameweek_daily,
        name_time_session_map_df,
        how="inner",
        on="Session name",
    )
    ...
    df_plot_weekly = df_stats_gameweek_daily.drop(
        ["Maximum speed [km/h]", "Start time"], axis=1
    )
    ...
    with PdfPages(f"Weekly Physical Report {date1} to {date2}.pdf") as pdf:
        plot_previous = plot_physical_volume(
            df_plot_clean_previous,
            f"Physical Load\n{date1} to {date2}",
        ...
        plot_weekly_daily_load = plot_bars(
            df_plot_weekly,
        ...
        # # ## TABLES
        # # # volume table
        volume_table = plot_table(
            df_team_volume_table,
    ...
    with open(f"Weekly Physical Report {date1} to {date2}.pdf", "rb") as report:
        st.download_button(
            "Download Report", report, f"Weekly Physical Report {date1} to {date2}.pdf"
        )
    ...
    if account == "M":
        ...
        # Volume
        altair_weekly_volume = plot_altair_scatter(
            df_altair_plot,
            title="Weekly Volume Load",
            x="Total Distance (km):Q",
            y="HSR distance (>20km/h):Q",
            tooltip=[
                "athlete_name:N",
                "Total Distance (km):Q",
                "HSR distance (>20km/h):Q",
            ],
            color="position_name:N",
            label="athlete_name:N",
            width=600,
            height=600,
            size=200,
        )
        ...
        # Intensity
        altair_intensity_plot = plot_altair_scatter(
            df_altair_plot,
            title="Weekly Intensity Load",
            x="Sprint distance (>25km/h):Q",
            y="Sprint efforts (>25km/h):Q",
            tooltip=[
                "athlete_name:N",
                "Sprint distance (>25km/h):Q",
                "Sprint efforts (>25km/h):Q",
                "Max Speed (km/h):Q",
            ],
            color="position_name:N",
            width=600,
        ...
        # Mechanical
        altair_mechanical_plot = plot_altair_scatter(
            df_altair_plot,
            title="Weekly Intensity Load",
            x="Acc (>3m/s):Q",
            y="Dec (>-3m/s):Q",
        ...
        altair_weekly_volume = plot_altair_scatter(
            ...
            title="Weekly Volume Load",
            x="Total Distance (km)",
            y="HSR distance (>19km/h)",
            tooltip=[
                "athlete_name:N",
                "Total Distance (km):Q",
                "HSR distance (>19km/h):Q",
            ],
            color="position_name:N",
            label="athlete_name:N",
            width=600,
        ...
        #                                             tooltip=["athlete_name:N", "Sprint distance (>22.5km/h):Q", "Sprint efforts (>22.5km/h):Q"],
