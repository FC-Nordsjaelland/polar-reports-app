...
def clean_players_df(players):
    map_positions = {"Forsvar":"Centre Back", "Midtbane": "Midfielder", "Angriber":"Attacker"}
...
def preprocess(data, set_date, team_players, session_name, account):
    ...
    if account == "M":
        ...
        data = pd.concat([data, pd.Series(data['Duration'].astype(str).apply(hms_to_m), name='Duration_min')], axis=1)
        ...
        data = pd.merge(data, team_players[['full_name', 'role']], left_on="Player name", right_on='full_name')
        data =  data.drop(['full_name'], axis=1)
        data = data.rename(columns={"role":'position_name', 'Player name':"athlete_name", 'Duration_min':'Minutes', 'Total distance [m]':"Total Distance"})
        sum_hr = data['Time in HR zone 4 (80 - 89 %)'] + data['Time in HR zone 5 (90 - 100 %)']
        data["HR (>85%)"] = sum_hr
        data = data.drop(['Time in HR zone 4 (80 - 89 %)', 'Time in HR zone 5 (90 - 100 %)'], axis=1)
        data = data[[
                'athlete_name', 
                'position_name',
                'Minutes',
...
def clean_df(df, volume=False, intensity=False):
    ...
    for pos in positions:
        df.loc[(f"{pos}", f"Åvg. {pos}"), :] = df_group.loc[(f"{pos}", "Åvg. Position"), :]
